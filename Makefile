...
# --- Phase 1: FAN-OUT (parallel branches read codebase.txt) ---
...
# --- Phase 2: LOCAL SUMMARIZATION (exactly 5 bullets each) ---
...
# Planner sees both the raw concatenated summary and the refined report,
# so it can keep detail the refine pass pruned while still benefitting from
# the dedupe / root-cause work.
